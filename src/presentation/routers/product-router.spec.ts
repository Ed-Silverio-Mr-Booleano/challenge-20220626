...
describe('Product Router', () => {
  ...
  describe('GET /products/:code', () => {
    test('should return 200 with data', async () => {
      ...
      jest.spyOn(mockGetOneProductsUseCase, 'execute').mockImplementation(async () => await Promise.resolve(ExpectedData))
      const response = await request(server).get('/products/:code')
      expect(response.status).toBe(200)
      expect(mockGetOneProductsUseCase.execute).toBeCalledTimes(1)
      expect(response.body).toStrictEqual(ExpectedData)
    })
    test('Should return 500 on use case error', async () => {
      jest.spyOn(mockGetOneProductsUseCase, 'execute').mockImplementation(async () => await Promise.reject(Error()))
      const response = await request(server).get('/products/:code')
      expect(response.status).toBe(500)
      expect(response.body).toStrictEqual({ message: 'Error fetching data' })
    })
